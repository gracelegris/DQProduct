...
txt_cov_flags              <- "Admin coverage flagged where values exceed 100% (orange) or change by more than ±{pct_threshold * 100} percentage points from the previous year (red), {min_yr_plots}–{rev_yr}.\n\nTwo types of data quality issues are highlighted: coverage values above 100%, which suggest errors in the reported number of vaccinations or the denominator used, and year-on-year changes greater than 10 percentage points, which may reflect inconsistent reporting rather than true changes in vaccination coverage."
...
txt_stockout_table         <- "Years in which a national or subnational vaccine stockout was recorded.\n\nThis table provides context for interpreting the numerator and denominator trends and flags in the following slides, as stockouts may contribute to reduced service delivery and legitimate drops in coverage that may be misinterpreted as data quality issues."
...
txt_denom_line <- "Target population (denominator) by vaccine and year. Points are colored by the magnitude of year-on-year change: green for less than 5%, orange for 5–10%, and red for greater than 10%. Black points indicate there was no denominator data in the previous year (unable to calculate percent change). Light blue shading highlights years where the direction of change switches unexpectedly.\n\nTarget populations should follow a smooth, steady trajectory. Sudden directional switches are demographically unnatural and typically indicate administrative errors, shifting census boundaries, or changes in reporting sources.\n\nGenuine switches are rare and generally limited to extreme disruptions such as humanitarian crises, cross-border migrations, or very small island populations."
txt_births_si              <- "Comparison of trends in live births and surviving infants, {min_yr_plots}–{rev_yr}.\n\nLive births is used is the denominator for BCG and HepBB.\n\nSurviving infants is used as the denominator for other vaccines given in the first year of life.\n\nAdmin target populations are reported by the country, while the United Nations Population Division (UNPD) produces the World Population Prospects (WPP) data."
txt_denom_pct_change       <- "Number of children in the live births (BCG) and surviving infants (DTP1) administrative populations by year. Points are colored by the magnitude of year-on-year change: green for less than 5%, orange for 5–10%, and red for greater than 10%. Black points indicate there was no denominator data in the previous year (unable to calculate percent change). Light blue shading highlights years where the direction of change switches unexpectedly.\n\nUnder normal circumstances, target populations should follow a smooth, steady trajectory rather than fluctuating. Sudden switches in direction, such as an increasing population abruptly decreasing the following year, are demographically unnatural and typically indicate administrative errors, shifting census boundaries, or changes in reporting data sources without historic time series adjustment.\n\nGenuine switches in growth direction are rare and generally limited to extreme disruptions like humanitarian refugee crises, cross-border migrations, or very small island populations."
txt_coadmin_schedule_title <- "Coverage of Vaccines Recommended at the Same Age"
txt_dropout_title          <- "Vaccine Dropout"
txt_dropout                <- "Admin coverage for key vaccine pairs (DTP1→DTP3, DTP1→MCV1) with dropout rate shown as triangles. High dropout may indicate supply, access, or demand barriers. \n\nA normal dropout rate means more children received DTP1 than DTP3 or MCV1, which is expected. A negative dropout rate means more children received DTP3/MCV1 than DTP1.\n\nFor DTP1–DTP3, negative dropout almost certainly reflects data quality issues, as a child cannot receive the third dose without first receiving the first dose.\n\nWhile a child can receive MCV1 without DTP1, negative dropout is generally unusual as MCV1 is recommended later in infancy. Negative dropout may occur in instance of outbreak response, but campaigns often include older children, which may contaminate the numerator and lead to misinterpretation."
...
txt_coadmin_bars           <- "Number of children vaccinated with vaccines co-administered at the {cluster_name} visit per the national schedule. {coadmin_labels[cluster_name]}"
...
txt_num_change_explanation <- "Raw counts of children vaccinated (numerator) by vaccine and year. Points are colored by the percent change from previous year in the number of children vaccinated.\n\nGreen point = stable year-to-year count of children vaccinated.\n\nOrange/red point = high fluctuation in number of children vaccinated, which may indicate data quality issues.\n\nBlack point = first year of available numerator data for that vaccine."
txt_numerator_bars <- "This chart shows number of children vaccinated (numerator) by vaccine and year.\n\nBlack diagonal lines indicate years with a national- or subnational-level vaccine stockout, which may explain any drops in the number of children vaccinated."
txt_numerator_bars_selection <- "Number of children vaccinated (numerator) by vaccine (DTP1, DTP3, and MCV1 only) and year.\n\nBlack diagonal lines indicate years with a vaccine stockout, which may explain any drops in the number of children vaccinated."
txt_caption_wpp_source <- paste0("Data from ", wpp_rev_yr, " World Population Prospects (WPP) revision.")

txt_timepoint_6weeks  <- "6-week"   # translated per language
txt_timepoint_10weeks <- "10-week"
txt_timepoint_14weeks <- "14-week"
txt_timepoint_2months <- "2-month"
txt_timepoint_4months <- "4-month"
txt_subtitle_coadmin  <- paste0("Vaccines administered at the {cluster_name} visit")